...
@section('content')
    <div class="container">
        ...
        <a href="{{ route('tarifs.create') }}" class="btn btn-dark">Ajouter un Activté</a>
        <br><br>
        ...
        <div class="table-responsive shadow-sm rounded-3">
            <table class="table caption-top text-capitalize">
                <thead>
                    <tr>
                        <th scope="col " class="bg-secondary text-white border-0 border-bottom-1">Type d'activité</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="bg-white border-0">{{ $tarif->activite }}</td>
                        <td class="bg-white border-0">{{ $tarif->prix_unitaire }} Ar</td>
                        <td class="bg-white border-0">
                            <a href="{{ route('tarifs.edit', $tarif->tarif_id) }}" class="nav-link">
                        ...
                        <td class="bg-white border-0 text-center">
                            <form action="{{ route('tarifs.destroy', $tarif->tarif_id) }}" method="post" style="display: inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-danger" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet activité?')">
